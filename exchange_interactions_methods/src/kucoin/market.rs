...
use exchange_interactions_adapters::kucoin::{KucoinHttpUrl, KucoinOption};
use jiff::Timestamp;
...
pub mod futures {
	...
	// prices {{{
	pub(in crate::kucoin) async fn prices(
		client: &exchange_interactions_adapters::Client,
		pairs: Option<Vec<Pair>>,
		_recv_window: Option<std::time::Duration>,
	) -> ExchangeResult<BTreeMap<Pair, f64>> {
	...
	// exchange_info {{{
	pub(in crate::kucoin) async fn exchange_info(client: &exchange_interactions_adapters::Client, _recv_window: Option<std::time::Duration>) -> ExchangeResult<ExchangeInfo> {
		let options = vec![KucoinOption::HttpUrl(KucoinHttpUrl::Futures)];
		let response: ContractsActiveResponse = client.get("/api/v1/contracts/active", &json!({}), options).await?;
...
// prices {{{
pub(super) async fn prices(client: &exchange_interactions_adapters::Client, pairs: Option<Vec<Pair>>, _recv_window: Option<std::time::Duration>) -> ExchangeResult<BTreeMap<Pair, f64>> {
...
// exchange_info {{{
pub(super) async fn exchange_info(client: &exchange_interactions_adapters::Client, _recv_window: Option<std::time::Duration>) -> ExchangeResult<ExchangeInfo> {
	let options = vec![KucoinOption::HttpUrl(KucoinHttpUrl::Spot)];
